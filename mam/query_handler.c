...
int copy_to_trait_struct_from_array(path_traits* path, char* array, int index) {
	if(TRACE_QH_FLOW) { trace_log("ENTERING: copy_to_trait_struct_from_array"); }
	...
	if(TRACE_QH_FLOW) { trace_log("LEAVING: copy_to_trait_struct_from_array"); }
	return index;	
}

int copy_to_addr_list_struct_from_array(ip_addr_ptr head, char* array, int index) {
	if(TRACE_QH_FLOW) { trace_log("ENTERING: copy_to_addr_list_struct_from_array"); }
	...
	if(TRACE_QH_FLOW) { trace_log("LEAVING: copy_to_addr_list_struct_from_array"); }
	return index;
}

path_traits* convert_reply_to_struct(path_traits* head, char* array) {
	if(TRACE_QH_FLOW) { trace_log("ENTERING: convert_reply_to_struct"); }
	if(array[0] != TERMINATION && array[0] != NULL_CHAR) {
		head->next = NULL;
		int index = copy_to_trait_struct_from_array(head, array, 0);
		index = skip_index_passed_delimiters(array, index);
		while(array[index] != TERMINATION && array[index] != NULL_CHAR) {
		...
		if(TRACE_QH_FLOW) { trace_log("LEAVING: convert_reply_to_struct, returning reply"); }
		return head;
	}
	free(head);
	if(TRACE_QH_FLOW) { trace_log("LEAVING: convert_reply_to_struct, returning NULL"); }
	return NULL;
}

query_addrs* convert_query_to_struct(query_addrs* addrs, char* array) {
	if(TRACE_QH_FLOW) { trace_log("ENTERING: convert_query_to_struct"); }
	...
	if(TRACE_QH_FLOW) { trace_log("LEAVING: convert_query_to_struct"); }
	return addrs;
}
...
int copy_addr_list_to_array(ip_addr_ptr addr, char* array, int index) {
	if(TRACE_QH_FLOW) { trace_log("ENTERING: copy_addr_list_to_array"); }
	ip_addr_ptr probe = addr;
	while(probe != NULL) {
		copy_addr_to_array(probe->addr, array, index);
		probe = probe->next;
	}
	if(TRACE_QH_FLOW) { trace_log("LEAVING: copy_addr_list_to_array"); }
...
void push_query_snd_addr(char* addr) {
	if(TRACE_QH_FLOW) { trace_log("ENTERING: push_query_snd_addr"); }
	...
	if(TRACE_QH_FLOW) { trace_log("LEAVING: push_query_snd_addr"); }
}

void push_query_rcv_addr(char* addr) {
	if(TRACE_QH_FLOW) { trace_log("ENTERING: push_query_rcv_addr"); }
	if(adding_snd) { adding_snd = 0; query_index = add_list_end(query_to_send, query_index); }
	if(query_index < DATA_LIMIT - 50) {
		query_index = copy_addr_to_array(addr, query_to_send, query_index);
	}
	else if(TRACE_ERRORS) { trace_log("QUERY ENTRY SKIPPED: total query to big!"); }
	if(TRACE_QH_FLOW) { trace_log("LEAVING: push_query_rcv_addr"); }
}
...
void push_reply_addr_pair(char* snd_addr, char* rcv_addr, int srtt, int jitt, int loss, int rate) { 
	if(TRACE_QH_FLOW) { trace_log("ENTERING: push_reply_addr_pair"); }
	...
	if(TRACE_QH_FLOW) { trace_log("LEAVING: push_reply_addr_pair"); }
}
...
void commit_query(char* query) { 
	if(TRACE_QH_FLOW) { trace_log("ENTERING: commit_query"); }
	...
	reset_query();
	if(TRACE_QH_FLOW) { trace_log("LEAVING: commit_query"); }
}
...
query_addrs* fetch_query() { 
	if(TRACE_QH_FLOW) { trace_log("ENTERING: fetch_query"); }
	...
	if(recv > 0) { 
		if(TRACE_QH_FLOW) { trace_log("LEAVING: fetch_query, returning query"); }
		return convert_query_to_struct(malloc(sizeof(query_addrs)), query);
	}
	if(TRACE_QH_FLOW) { trace_log("LEAVING: fetch_query, returning NULL"); }
	return NULL;
}
...
void commit_reply() { 
	if(TRACE_QH_FLOW) { trace_log("ENTERING: commit_reply"); }
	if(sniffer_sockfd == 0) { setup_query_listener(); }
	...
	reset_reply();
	if(TRACE_QH_FLOW) { trace_log("LEAVING: commit_reply"); }
}
